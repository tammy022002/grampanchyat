...
@Component({
  ...
  template: `
    <div class="page-header">
      <h2>Budget Master</h2>
      <button class="btn-primary" (click)="openForm()">+ Add Budget</button>
    </div>
    ...
    <div class="modal-overlay" *ngIf="showForm">
      <div class="modal-content">
        <div class="modal-header">
          <h3>{{ editingId ? 'Edit' : 'New' }} Budget Entry</h3>
...
export class BudgetListComponent {
  ...
  columns: GridColumn[] = [
    ...
    { field: 'estimated_receipts', header: 'Est. Receipts', type: 'currency' },
    { field: 'estimated_expenditure', header: 'Est. Expenditure', type: 'currency' },
  ...
  openForm(row?: any) {
    this.showForm = true;
    if (row) {
      this.editingId = row.budget_id; // real ID from DB
      this.form.patchValue(row);
    } else {
      this.editingId = null;
      this.form.reset();
      this.form.patchValue({ estimated_receipts: 0, estimated_expenditure: 0 });
    }
  }
  ...
  onGridAction(event: any) {
    if (event.action === 'Edit') {
      ...
      this.pdfService.generateRowReceipt('budget', this.columns, event.row);
    }
  }
  ...
  onSubmit() {
    if (this.form.valid) {
      ...
      if (this.editingId) payload.budget_id = this.editingId;
      
      this.apiService.post('/finance/budgets', payload).subscribe({
        next: () => {
          this.loading = false;
          this.closeForm();
